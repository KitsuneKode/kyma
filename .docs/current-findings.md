...
- webhook-driven room sync exists, but the `processing -> report generated` pipeline still needs Inngest
...
- wire `processing` sessions into Inngest so transcript + artifacts become report/evidence automatically
