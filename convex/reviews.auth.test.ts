...
describe('getCandidateReviewDetail auth matrix', () => {
  ...
  /**
   * Regression: processing write key must never unlock the public recruiter
   * review query (pre-fix IDOR via optional `processingKey` arg).
   */
  test('rejects unauthenticated callers even when processing write key is set', async () => {
    process.env.KYMA_PROCESSING_WRITE_KEY = 'pipeline-secret'
    const t = harness()
    const { sessionId } = await seedReviewSession(t)
  ...
  test('rejects candidate identity even when processing write key is set', async () => {
    process.env.KYMA_PROCESSING_WRITE_KEY = 'pipeline-secret'
    const t = harness()
    const { sessionId } = await seedReviewSession(t)
    const asCandidate = t.withIdentity(CANDIDATE)
